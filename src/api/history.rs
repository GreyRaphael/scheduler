...
async fn list_all_history(
    ...
    let paged = conn
        .interact(move |c| crate::db::history_repo::list_all_history(c, filter))
        .await
...
async fn clear_all_history(
    ...
    let conn = state.pool.get().await.map_err(AppError::internal)?;
    let count = conn
        .interact(move |c| crate::db::history_repo::clear_all_history(c))
        .await
        .map_err(AppError::internal)?
        .map_err(AppError::internal)?;
...
async fn get_history_detail(
    ...
    let conn = state.pool.get().await.map_err(AppError::internal)?;
    let hist_opt = conn
        .interact(move |c| crate::db::history_repo::get_history(c, uuid))
        .await
        .map_err(AppError::internal)?
        .map_err(AppError::internal)?;
    ...
    match hist_opt {
        Some(h) => Ok(Json(h)),
        None => Err(AppError::not_found("History record not found")),
    }
}
...
async fn list_task_history(
    ...
    let uuid = Uuid::parse_str(&task_id).map_err(AppError::bad_request)?;
    let conn = state.pool.get().await.map_err(AppError::internal)?;
    let paged = conn
        .interact(move |c| {
            crate::db::history_repo::list_task_history(c, uuid, filter.page, filter.per_page)
        })
        .await
        .map_err(AppError::internal)?
        .map_err(AppError::internal)?;
